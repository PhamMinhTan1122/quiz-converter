...
def write_to_excel(data_list, excel_filename):
    ...
    for i in range(0, len(data_list), 5):
        ...
        if len(data_list) >= i + 4:
            ...
            ws[f"A{row}"] = data_list[i] # The question
            ws[f"C{row}"] = data_list[i+1]# Option A
            ws[f"D{row}"] = data_list[i+2] # Option B
            ws[f"E{row}"] = data_list[i+3] # Option C
            ws[f"F{row}"] = data_list[i+4] # Option D
